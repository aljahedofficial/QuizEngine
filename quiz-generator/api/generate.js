// Serverless function placeholder for quiz generation.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const prompt = `
    You are an expert exam setter specializing in ${subject}.
    Analyze the following text extracted from a PDF document:

    ${extractedText}

    Tasks:
    1. Generate 10 multiple-choice questions (MCQs) focusing on ${subject}.
    2. Write all questions, options, and explanations in ${language}.

    Return strictly a raw JSON array following this structure:
    [
      {
